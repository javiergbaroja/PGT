...
class CellDetDataset_Lizard_custom(CustomDataset):
    CLASSES = ('Others', "Healthy Epithelial", "Malignant Epithelial")
    ...
    def det(self, pred_centroid, pred_inst_type, img_name, img_idx):
        ...
        self.true_idx_offset = (
            self.true_idx_offset + self.true_inst_type_all[-1].shape[0] if img_idx != 0 else 0
        )
        self.pred_idx_offset = (
            self.pred_idx_offset + self.pred_inst_type_all[-1].shape[0] if img_idx != 0 else 0
        )
    ...
    def evaluate(self,
        ...
        self.paired_all = []
        self.unpaired_true_all = []
        self.unpaired_pred_all = []
        self.true_inst_type_all = []
        self.pred_inst_type_all = []
        ...
        for idx, result in enumerate(results):
            ...
            # example: patches/glas_54/id_0_x_0_256_y_0_256.png
            img_name_oral, patch_name = os.path.split(img_name)
            ret = re.match(r'(.*)_(.*)_(.*)_(.*)_(.*)_(.*)_(.*).png$', patch_name)
            img_name_oral = os.path.basename(img_name_oral)
            x_start = int(ret.group(3))
            y_start = int(ret.group(6))
            # x_end = int(ret.group(4))
            # y_end = int(ret.group(5))
            ...
            if idx == 0:
                ...
                self.det(pred_centroid, pred_inst_type, img_name_oral_last, img_idx)
                img_name_oral_last = img_name_oral
                # pred info
                pred_centroid = []
                pred_inst_type = []
            ...
            for i in range(len(self.cat_ids)):
                classes = result[i][np.where(result[i][:, 2] > 0.5)]
                classes = classes[:, :2]
                classes[:, 0] += y_start
                classes[:, 1] += x_start
        ...
        paired_all = np.concatenate(self.paired_all, axis=0) if len(self.paired_all) else np.empty((0, 1))
        ...
        paired_true_type = true_inst_type_all[paired_all[:, 0]] if len(self.paired_all) else np.empty((0, 1))
        paired_pred_type = pred_inst_type_all[paired_all[:, 1]] if len(self.paired_all) else np.empty((0, 1))
...
img = mmcv.imread(os.path.join("/data2/huangjunjia/coco/BRCA/COCO_BRCA/BRCA_Test", img_name))
plt.imshow(img)
idx=0
plt.scatter(true_centroid[true_inst_type==idx+1][:,0],true_centroid[true_inst_type==idx+1][:,1],s=5,color=(1,0,0))
plt.scatter(result[idx][np.where(result[idx][:, 2] > 0.6)][:,0], result[idx][np.where(result[idx][:, 2] > 0.6)][:, 1], s=5, color=(0,1,0))
plt.show()
'''
